...
class Streaming(QObject):
    ...
    def __init__(self, parent, iface, chunks, playlistUrl, mimeType, encoding):
        ...
        if isMimeTypeRaster(self.mimeType, True) is not None:
        ...
        self.timer.timeout.connect(partial(self.fetchPlaylist,
                                           self.playlistUrl))
    ...
    def handlePlaylist(self, reply):
        ...
        if self.DEBUG:
            print(str(self.__loadedChunks) + " chunks loaded" + (
    ...
    def handleChunk(self, reply):
        ...
        if self.chunks:
            ...
            self.parent.lblProcess.setText(
                "Downloading chunks... (" + str(
                    self.__deliveredChunks + 1) + "/" + str(self.chunks) + ")")
        ...
        if self.DEBUG:
            print("READY to be loaded (", resultFile,
                  ", chunkId:", chunkId, ")")
    ...
    def loadData(self, resultFile, chunkId):
        ...
        if isMimeTypeVector(self.mimeType, True) is not None:
            # Memory layer:
            geometryTypes = ["Point", "LineString", "Polygon", "Unknown",
                             "NoGeometry"]
            ...
            if not self.allChunksDelivered():
                ...
                while provider.nextFeature(inFeat):
                    ...
                    featList = self.extractAsSingle(
                        self.__geometryType, inGeom) if self.__bGeomMulti else [inGeom]
                    for geom in featList:
                        self.addTempGeometry(chunkId,
                                             self.__geometryType, geom)
            else:
                self.finishLoading()
    ...
    def createTempGeometry(self, chunkId, geometryType):
        ...
        if geometryType == "Polygon":
            self.__tmpGeometry[chunkId] = QgsRubberBand(self.iface.mapCanvas(),
                                                        True)
