    ...
    if rLayer.dataProvider().name() == 'ogr' or rLayer.dataProvider().name() == 'gdal':
        ...
        QMessageBox.critical(
            ...
            QApplication.translate(
                "QgsWps", 'Datatype %s of layer %s is not supported!' % (
                    rLayer.dataProvider().name(), rLayer.name())))
        return ''
    ...
    if error != QgsVectorFileWriter.NoError:
        ...
        if (not myFile.open(QIODevice.ReadOnly | QIODevice.Text)):
            QMessageBox.information(
                None, '', QApplication.translate("QgsWps",
                                                 "File open problem"))
...
class ExecutionRequest(QObject):
    ...
    def addPlainTextInput(self, identifier, text):
        ...
        self.request += "<wps:ComplexData>" + htmlescape(
            text) + "</wps:ComplexData>\n"
...
#        self.request += "</wps:ComplexData>\n"
    ...
    def addMultipleGeometryInput(self, identifier, mimeType, schema, encoding,
                                 gmldata, useSelected):
    ...
    def addLiteralDataInput(self, identifier, text):
        self.addExecuteRequestInputStart(identifier)
        self.request += "<wps:LiteralData>" + htmlescape(
            unicode(text)) + "</wps:LiteralData>\n"
        self.addExecuteRequestInputEnd()

    def addBoundingBoxInput(self, identifier, bboxArray):
        self.addExecuteRequestInputStart(identifier)
        self.request += '<wps:BoundingBoxData ows:dimensions="2">'
        self.request += '<ows:LowerCorner>' + bboxArray[0] + ' ' + bboxArray[
            1] + '</ows:LowerCorner>'
        self.request += '<ows:UpperCorner>' + bboxArray[2] + ' ' + bboxArray[
            3] + '</ows:UpperCorner>'
    ...
    def addReferenceOutput(self, identifier, mimeType, schema, encoding):
        ...
        self.request += "<ows:Identifier>" + htmlescape(
