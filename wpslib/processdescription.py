...
RASTER_MIMETYPES = [{"MIMETYPE": "image/tiff", "GDALID": "GTiff",
                    ...
                    {"MIMETYPE": "application/image-ascii-grass",
                     "GDALID": "GRASSASCIIGrid", "EXTENSION": "asc"}]
# All supported input vector formats [mime type, schema]
VECTOR_MIMETYPES = [{"MIMETYPE": "application/x-zipped-shp", "SCHEMA": "",
                     "GDALID": "ESRI Shapefile", "DATATYPE": "SHP",
                     "EXTENSION": "zip"},
                    {"MIMETYPE": "application/vnd.google-earth.kml+xml",
                     "SCHEMA": "KML", "GDALID": "KML", "DATATYPE": "KML",
                     "EXTENSION": "kml"},
                    ...
                    {"MIMETYPE": "application/json", "SCHEMA": "JSON",
                     "GDALID": "GEOJSON", "DATATYPE": "JSON",
                     "EXTENSION": "json"},
                    {"MIMETYPE": "application/geojson",
                     "SCHEMA": "GEOJSON", "GDALID": "GEOJSON",
                     "DATATYPE": "GEOJSON", "EXTENSION": "geojson"}]
# mimeTypes for streaming
PLAYLIST_MIMETYPES = [{"MIMETYPE": "application/x-ogc-playlist+",
                       "SCHEMA": "", "GDALID": "", "DATATYPE": "PLAYLIST",
                       "EXTENSION": "txt"}]
...
def isMimeTypeRaster(mimeType, ignorePlaylist=False):
    ...
    if not ignorePlaylist:
        if isMimeTypePlaylist(mimeType) is not None:
            return None
...
def allowedValues(aValues):
    ...
    if v_range_element.size() > 0:
        min_val = getOwsElement(
            value_element, "MinimumValue").at(0).toElement().text()
        max_val = getOwsElement(
        ...
        try:
            ...
            QMessageBox.critical(
                ...
                QApplication.translate("QgsWps",
                                       "Maximum allowed Value is too large"))
...
StringInput = namedtuple('StringInput',
                         'identifier title minOccurs defaultValue')
TextInput = namedtuple('TextInput', 'identifier title minOccurs dataFormat')
SelectionInput = namedtuple('SelectionInput',
                            'identifier title, minOccurs valList')
VectorInput = namedtuple('VectorInput',
                         'identifier title minOccurs dataFormat')
MultipleVectorInput = namedtuple('MultipleVectorInput',
                                 'identifier title minOccurs dataFormat')
RasterInput = namedtuple('RasterInput',
                         'identifier title minOccurs dataFormat')
MultipleRasterInput = namedtuple('MultipleRasterInput',
                                 'identifier title minOccurs dataFormat')
FileInput = namedtuple('FileInput', 'identifier title minOccurs dataFormat')
MultipleFileInput = namedtuple('MultipleFileInput',
                               'identifier title minOccurs dataFormat')
...
class ProcessDescription(QObject):
    ...
    def _describeProcessFinished(self):
        ...
        if cookies is not None:
            QMessageBox.information(
                None, '', "the first time to use this server")
    ...
    def _parseProcessInputs(self):
        ...
        for i in range(dataInputs.size()):
            ...
            if complexData.size() > 0:
                ...
                if isMimeTypeVector(complexDataFormat["MimeType"]) is not None:
                    # Since it is a vector, choose an appropriate GML version
                    complexDataFormat = self.getSupportedGMLDataFormat(
                        inputIdentifier)

                    if complexDataFormat is None:
                        QMessageBox.warning(
                            None, QApplication.translate("QgsWps", "Error"),
                            QApplication.translate("QgsWps",
                                                   "The process '{0}' does not seem to support GML for the parameter '{1}', which is required by the QGIS WPS client.").format(self.processIdentifier,  inputIdentifier))
                        return 0
                    ...
                    if maxOccurs == 1:
                        ...
                        self.inputs.append(MultipleVectorInput(
                            inputIdentifier, title, minOccurs,
                            complexDataFormat))
                elif isMimeTypeText(complexDataFormat["MimeType"]) is not None:
                    # Text inputs
                    self.inputs.append(TextInput(inputIdentifier, title,
                                                 minOccurs, complexDataFormat))
                elif isMimeTypeRaster(complexDataFormat["MimeType"]) is not None:
                ...
                elif isMimeTypePlaylist(complexDataFormat["MimeType"]) is not None:
                    # Playlist (text) inputs
                    self.inputs.append(TextInput(inputIdentifier,
                                                 title, minOccurs,
                                                 complexDataFormat))

                elif isMimeTypeFile(complexDataFormat["MimeType"]) is not None:
                    if maxOccurs == 1:
                        self.inputs.append(FileInput(inputIdentifier,
                                                     title, minOccurs,
                                                     complexDataFormat))
                    else:
        ...
        for i in range(dataInputs.size()):
            ...
            if literalData.size() > 0:
                ...
                if aValues.size() > 0:
                    ...
                    if len(valList) > 0:
                        if len(valList[0]) > 0:
                            self.inputs.append(SelectionInput(inputIdentifier,
                                                              title, minOccurs,
                                                              valList))
                        else:
                            self.inputs.append(StringInput(
                                inputIdentifier, title, minOccurs, str(
                                    valList)))
                else:
        ...
        for i in range(dataInputs.size()):
            ...
            if bBoxData.size() > 0:
                ...
                defaultCrs = defaultCrsElement.elementsByTagName(
                    "CRS").at(0).toElement().attributeNS(
                        "http://www.w3.org/1999/xlink", "href")
                crsListe.append(defaultCrs)

                self.inputs.append(ExtentInput(inputIdentifier, title,
                                               minOccurs))
                ...
                for i in range(supportedCrsElements.size()):
                    crsListe.append(supportedCrsElements.at(
                        i).toElement().elementsByTagName("CRS").at(
                            0).toElement().attributeNS(
                                "http://www.w3.org/1999/xlink", "href"))

                self.inputs.append(CrsInput(inputIdentifier, title, minOccurs,
                                            crsListe))
    ...
    def _parseProcessOutputs(self):
        ...
        for i in range(dataOutputs.size()):
            ...
            if complexOutput.size() > 0:
                ...
                getSupportedMimeTypes(complexOutputTypeElement)
                if isMimeTypeVector(complexOutputFormat["MimeType"]) is not None:
                    self.outputs.append(VectorOutput(outputIdentifier, title,
                                                     complexOutputFormat))
                else:
                    self.outputs.append(RasterOutput(outputIdentifier, title,
                                                     complexOutputFormat))
    ...
    def getSupportedGMLVersion(self, dataIdentifier):
        ...
        if isGML3SupportedByOGR() and self.isDataTypeSupportedByServer(
                getBaseMimeType("GML3"), dataIdentifier):
            return "GML3"
        elif self.isDataTypeSupportedByServer(getBaseMimeType("GML2"),
                                              dataIdentifier):
            return "GML2"
        elif self.isDataTypeSupportedByServer(getBaseMimeType("GML"),
                                              dataIdentifier):
            return "GML"
        else:
            return ""
    ...
    def getSupportedGMLDataFormat(self, dataIdentifier):
        ...
        if supportedGML != "":
            return self.getDataTypeInfo(getBaseMimeType(supportedGML),
                                        dataIdentifier)
        else:
            return None
