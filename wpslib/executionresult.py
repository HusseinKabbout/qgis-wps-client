...
def decodeBase64(infileName, mimeType="", tmpDir=None):
    try:
        tmpFile = tempfile.NamedTemporaryFile(
            prefix="base64", suffix=getFileExtension(mimeType),
...
class ExecutionResult(QObject):
    ...
    def __init__(self, literalResultCallback, resultFileCallback,
                 successResultCallback, errorResultCallback, streamingHandler,
                 progressBar=None):
    ...
    def resultHandler(self, reply):
        """Handle the result of the WPS Execute request and add the outputs as new
           map layers to the registry or open an information window to show
           literal outputs."""
                ...
                if f_element.elementsByTagNameNS("http://www.opengis.net/wps/1.0.0", "Reference").size() > 0:
                    ...
                    if fileLink != '0':
                        if "playlist" in self.mimeType:  # Streaming based process?
                            ...
                            self.fetchResult(encoding, schema, fileLink,
                                             identifier)
                    ...
                    complexData = f_element.elementsByTagNameNS(
                        "http://www.opengis.net/wps/1.0.0", "ComplexData").at(
                            0).toElement()

                    # Get the mime type of the result
                    self.mimeType = complexData.attribute("mimeType", "0").lower()

                    # Get the encoding of the result, it can be used decoding base64
                    encoding = complexData.attribute("encoding", "").lower()
                    schema = reference.attribute("schema", "").lower()

                    if "playlist" in self.mimeType:
                        playlistUrl = f_element.elementsByTagNameNS(
                            "http://www.opengis.net/wps/1.0.0",
                            "ComplexData").at(0).toElement().text()
                        ...
                        QMessageBox.warning(
                            ...
                            QApplication.translate(
                                "QgsWps",
                                "WPS Error: The mimeType '" + self.mimeType + "' is not supported by this client"))
                    ...
                    QMessageBox.warning(
                        None, '',
                        QApplication.translate(
                            "QgsWps", "WPS Error: Missing reference or literal data in response"))
        else:
...
#                    self.errorHandler(resultXML)
    ...
    def fetchResult(self, encoding, schema, fileLink, identifier):
        ...
        self.theReply.finished.connect(
            partial(self.getResultFile,
                    identifier, self.mimeType, encoding, schema,
                    self.theReply))
