...
class WpsAlgorithm(GeoAlgorithm):
    ...
    def loadProcessDescription(self):
        ...
        if not os.path.exists(self.process.processDescriptionFile(self.wpsDescriptionFolder())):
            ...
            if self.process.identifier is None or self.process.identifier == "":
    ...
    def buildParametersDialog(self):
        for input in self.process.inputs:
            ...
            if inputType == VectorInput:
                ...
                self.addParameter(
                    ParameterMultipleInput(str(
                        input.identifier),
                        str(input.title),
                        ParameterVector.VECTOR_TYPE_ANY,
                        input.minOccurs == 0))
            elif inputType == StringInput:
                self.addParameter(ParameterString(str(input.identifier),
                                                  str(input.title)))
            elif inputType == TextInput:
                self.addParameter(ParameterString(str(input.identifier),
                                                  str(input.title)))
            elif inputType == RasterInput:
                self.addParameter(ParameterRaster(str(input.identifier),
                                                  str(input.title),
                                                  input.minOccurs == 0))
                ...
                self.addParameter(ParameterFile(str(input.identifier),
                                                str(input.title)))
            elif inputType == MultipleFileInput:
                pass  # Not supported
            elif inputType == SelectionInput:
                self.addParameter(ParameterSelection(str(input.identifier),
                                                     str(input.title),
                                                     input.valList))
            elif inputType == ExtentInput:
                self.addParameter(ParameterExtent(str(input.identifier),
                                                  str(input.title)))
            elif inputType == CrsInput:
                self.addParameter(ParameterCrs(str(input.identifier),
                                               "Projection", None))

        for output in self.process.outputs:
            outputType = type(output)
            if outputType == VectorOutput:
                self.addOutput(OutputVector(str(output.identifier),
                                            str(output.title)))
            elif outputType == RasterOutput:
                self.addOutput(OutputRaster(str(output.identifier),
                                            str(output.title)))
            elif outputType == StringOutput:
                self.addOutput(OutputString(str(output.identifier),
                                            str(output.title)))
    ...
    def defineProcess(self):
        ...
        for input in self.process.inputs:
            ...
            if inputType == VectorInput:
                ...
                if layer is None:
                    raise Exception(
                        "Couldn't extract layer for parameter '%s' from '%s'" % (input.identifier, value))
                    mimeType = input.dataFormat["MimeType"]
                data = createTmpGML(layer, useSelected, mimeType)
                request.addGeometryInput(
                    input.identifier, mimeType, input.dataFormat["Schema"],
                    input.dataFormat["Encoding"], data, useSelected)
            elif inputType == MultipleVectorInput:
                ...
                layer = dataobjects.getObjectFromUri(value, False)
                mimeType = input.dataFormat["MimeType"]
                request.addGeometryBase64Input(input.identifier, mimeType,
                                               layer)
            elif inputType == MultipleRasterInput:
        ...
        for output in self.process.outputs:
            ...
            if outputType == StringOutput:
                ...
                request.addReferenceOutput(output.identifier, mimeType,
                                           schema, encoding)
    ...
    def getLiteralResult(self, identifier, literalText):
        ...
        ProcessingLog.addToLog(
            ProcessingLog.LOG_INFO, identifier + ": " + literalText)
    ...
    def getResultFile(self, identifier, mimeType, encoding, schema, reply):
        ...
        resultFile = self.wps.handleEncoded(tmpFile, mimeType,
                                            encoding,  schema)
    ...
    def loadData(self, resultFile, mimeType, identifier):
        ...
        if isMimeTypeVector(mimeType) is not None:
            self.setOutputValue(identifier, resultFile)
        # Raster data
        elif isMimeTypeRaster(mimeType) is not None:
            self.setOutputValue(identifier, resultFile)

        # Text data
        elif isMimeTypeText(mimeType) is not None:
            text = open(resultFile, 'r').read()
            self.setOutputValue(identifier, text)

        # Everything else
        elif isMimeTypeFile(mimeType) is not None:
            text = open(resultFile, 'r').read()
            self.setOutputValue(identifier, text)
        ...
        # Everything else
            ...
            QMessageBox.information(None, QCoreApplication.translate(
