...
#                self.literalInputComboBoxList.append(self.tools.addLiteralComboBox("Supported CRS", inputIdentifier, crsListe, minOccurs,  self.dlgProcessScrollAreaWidget,  self.dlgProcessScrollAreaWidgetLayout))
        ...
        self.tools.addCheckBox(
            QCoreApplication.translate(
                "QgsWps", "Process selected objects only"),
    ...
    def defineProcess(self):
        ...
        if len(self.process.inputs) > 0:
            ...
            for textBox in self.complexInputTextBoxList:
                ...
                if textBox is None or str(textBox.document().toPlainText()) == "":
                    continue
                ...
                if isMimeTypePlaylist(self.mimeType) != None:
                    schema = self.inputDataTypeList[textBox.objectName()][
                        "Schema"]
                    encoding = self.inputDataTypeList[textBox.objectName()][
                        "Encoding"]

                    # Handle 'as reference' playlist
                    request.addReferenceInput(
                        textBox.objectName(), self.mimeType, schema, encoding,
            ...
            for comboBox in self.complexInputComboBoxList:
                # Do not add undefined inputs
                if comboBox is None or comboBox.currentText() == "<None>":
                    continue
                # TODO: Check for more types (e.g. KML, Shapefile, JSON)
...
#              try:
                if isMimeTypeVector(self.mimeType) is not None and encoding != "base64":
                    gmldata = createTmpGML(
                        ...
                        useSelected, self.process.getSupportedGMLVersion(
                            comboBox.objectName()))
                    request.addGeometryInput(
                        comboBox.objectName(), self.mimeType,
                        schema, encoding, gmldata, useSelected)
...
#                  return
            ...
            # Iterate over each seletced item
                for i in range(listWidgets.count()):
                    ...
                    if listWidget is None or listWidget.isSelected() is False or str(listWidget.text()) == "<None>":
                        continue
            ...
            for comboBox in self.literalInputComboBoxList:
                if comboBox is None or comboBox.currentText() == "":
                    continue
            ...
            for lineEdit in self.literalInputLineEditList:
                if lineEdit is None or lineEdit.text() == "":
                    continue
        ...
        if dataOutputs.size() > 0 or len(self.complexOutputComboBoxList) > 0:
            ...
            for comboBox in self.complexOutputComboBoxList:
                ...
                if comboBox is None or str(comboBox.currentText()) == "<None>":
                    continue
        ...
        self.wps = ExecutionResult(
            self.getLiteralResult, self.getResultFile, self.successResult,
            self.errorResult, self.streamingHandler, self.progressBar)
    ...
    def saveBookmark(self):
        ...
        QMessageBox.information(self.iface.mainWindow(),
                                ...
                                QCoreApplication.translate(
                                    "QgsWps",
                                    "The creation bookmark was successful."))
    ...
    def loadData(self, resultFile):
        ...
        # Text data
            ...
            self.tools.popUpMessageBox(QCoreApplication.translate(
                "QgsWps",
                'Process result (text/plain)'), text)
    ...
    def errorResult(self, exceptionHtml):
        ...
        QMessageBox.critical(
            self.iface.mainWindow(), "Exception report", exceptionHtml)
