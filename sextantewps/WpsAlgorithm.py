...
class WpsAlgorithm(GeoAlgorithm):
    ...
    def buildParametersDialog(self):
        for input in self.process.inputs:
            ...
            if inputType == VectorInput:
                self.addParameter(
                    ParameterVector(str(input.identifier),
                                    str(input.title),
                ...
                self.addParameter(
                    ParameterRaster(str(input.identifier),
                                    str(input.title), input.minOccurs == 0))
        ...
        for input in self.process.inputs:
            ...
            if inputType == VectorInput:
                ...
                request.addGeometryInput(
                    input.identifier, mimeType,
                    input.dataFormat["Schema"],
                ...
                request.addGeometryBase64Input(
                    input.identifier, mimeType, layer)
            elif inputType == MultipleRasterInput:
        ...
        for output in self.process.outputs:
            ...
            if outputType == StringOutput:
                ...
                request.addReferenceOutput(
                    output.identifier, mimeType, schema, encoding)
    ...
    def processAlgorithm(self, progress):
        ...
        self.wps = ExecutionResult(
            self.getLiteralResult, self.getResultFile, self.errorResult, None)
    ...
    def getLiteralResult(self, identifier, literalText):
        ...
        SextanteLog.addToLog(
            SextanteLog.LOG_INFO, identifier + ": " + literalText)
